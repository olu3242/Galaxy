...
describe('workflow step execution dispatcher', () => {
  ...
  it('hands notification steps to the notification worker and advances after durable enqueue', async () => {
    const q = queues();
    const client = { query: vi.fn() } as unknown as PoolClient;
    const execute = createStepExecutor(q);
    ...
    expect(result).toEqual({ disposition: 'advance', engine: 'notification' });
  ...
  it('schedules delay steps through the workflow queue', async () => {
    const q = queues();
    const client = { query: vi.fn() } as unknown as PoolClient;
    const execute = createStepExecutor(q);
    ...
    expect(q.workflow.add).toHaveBeenCalledWith(
      'advance-step',
      expect.objectContaining({ runId: context.workflowRunId, correlationId: context.correlationId }),
      { delay: 4500 },
    );
  });
