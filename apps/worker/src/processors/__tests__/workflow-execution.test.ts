...
function makePool(state: MockState = {}): Pool {
  ...
  const query = vi.fn((sql: string, params?: unknown[]): Promise<QueryResult<Record<string, unknown>>> => {
    ...
    if (sql.includes('FROM workflow_runs') && sql.includes('SELECT id, workflow_id')) {
      ...
      result = ok([{
        id: 'task-1', organization_id: ORG, workflow_run_id: RUN_ID, title: 'Wait',
        description: null, status: 'pending', priority: 'medium', assigned_to: null,
        created_by: ACTOR, due_at: null, completed_at: null, correlation_id: 'corr-1',
        data: { workflowStepId: STEP_1 }, created_at: '2026-09-13T00:00:00.000Z',
  ...
  return { connect: vi.fn().mockResolvedValue(client), query: vi.fn().mockResolvedValue(ok([])) } as unknown as Pool;
}
...
describe('workflow-execution runtime convergence', () => {
  it('starts the workflow at the first concrete step', async () => {
    ...
    expect(calls.some(([sql]) => sql.includes("UPDATE workflow_runs SET status = 'waiting'"))).toBe(true);
  });
  ...
  it('resumes a waiting engine step and advances exactly once', async () => {
    ...
    await createWorkflowProcessor(pool)(makeJob('resume-step', {
  ...
  it('ignores a duplicate completion receipt without advancing again', async () => {
    ...
    const calls = (await getClientQuery(pool)).mock.calls as [string, unknown[]][];
    expect(calls.some(([sql]) => sql.includes('INSERT INTO workflow_execution_receipts'))).toBe(true);
    expect(calls.some(([sql]) => sql.includes("SET status = 'completed', completed_at = NOW()"))).toBe(false);
  });
