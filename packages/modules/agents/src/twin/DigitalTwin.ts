...
export class DigitalTwin {
  ...
  async capture(organizationId: string): Promise<DigitalTwinSnapshot> {
    ...
    const [workflowStatusResult, workflowCompletedResult, approvalResult, memberResult, agentResult] =
      await Promise.all([
    ...
    const approvalPending =
      approvalRow !== undefined ? parseInt(approvalRow.pending, 10) : 0;
    const approvalOverdue =
      approvalRow !== undefined ? parseInt(approvalRow.overdue, 10) : 0;
    ...
    const memberDepartments =
      memberRow !== undefined ? parseInt(memberRow.departments, 10) : 0;

    const activeExecutions =
      agentRow !== undefined ? parseInt(agentRow.active_executions, 10) : 0;
    const agentCompleted =
      agentRow !== undefined ? parseInt(agentRow.completed_last24h, 10) : 0;
    const agentFailed =
      agentRow !== undefined ? parseInt(agentRow.failed_last24h, 10) : 0;
