...
export class CustomerHealthService {
  ...
  async computeHealth(orgId: string): Promise<CustomerHealth> {
    ...
    const activityResult = await this.pool.query<{ last_activity: string | null; event_count: string }>(
      `SELECT MAX(created_at) AS last_activity, COUNT(*)::text AS event_count
       FROM audit_logs WHERE organization_id = $1`,
      [orgId],
    );

    const paymentResult = await this.pool.query<{ last_payment: string | null; failed_count: string }>(
    ...
    const engagementScore = daysSinceActivity < 1 ? 33 : daysSinceActivity < 7 ? 25 : daysSinceActivity < 30 ? 15 : 0;
